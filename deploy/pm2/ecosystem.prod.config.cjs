...
// /var/www/www.hammerspace.co.uk/.env, which the install scripts source
// into the shell before calling `pm2 start`/`pm2 restart`.
//
// PORT is hardcoded here (not left to .env) so prod is guaranteed to run on
// 3001 regardless of what's in that shared .env file, and can never
// accidentally collide with the beta app's port. This must stay in sync
// with the `upstream` port in deploy/nginx/www.hammerspace.co.uk.conf.
module.exports = {
	apps: [
		{
			name: "www-hammerspace",
			script: "server.js",
			cwd: "/var/www/www.hammerspace.co.uk/standalone",
			interpreter: "node",
			instances: 1,
			exec_mode: "fork",
			autorestart: true,
